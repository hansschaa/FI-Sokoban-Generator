...
def evaluate():
    ...
    try:
        ...
        with torch.no_grad():
            logits_old = classifier_old(batch_tensor)
            probs_old = torch.sigmoid(logits_old)
            
            logits_new = classifier_new(batch_tensor)
            probs_new = torch.sigmoid(logits_new)
        
        # Determine solvability using dynamically loaded calibrated threshold
        is_solvable_old = (probs_old >= CLASSIFIER_THRESHOLD)
        is_solvable_new = (probs_new >= CLASSIFIER_THRESHOLD)
        
        # Log to file for verification
        with open("scratch/classifier_compare.txt", "a") as f:
            f.write(f"{len(boards_data)},{is_solvable_old.sum().item()},{is_solvable_new.sum().item()}\n")

        # Use the NEW classifier to drive the GA (or old, doesn't matter for the first few generations we just want counts)
        probs = probs_new
        is_solvable = is_solvable_new
